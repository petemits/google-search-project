...
class GoogleSearchAPI {
    ...
    detectLanguage(code) {
        if (code.includes('console.log') || code.includes('require(') || code.includes('import ') || code.includes('function ')) {
            return 'javascript';
    ...
    async startCodeExecutionMode() {
        ...
        console.log(chalk.white('   Supported languages: JavaScript, Python, Shell'));
        console.log(chalk.white('   Type "mode" to switch back to search'));
        console.log(chalk.gray('─'.repeat(40)));

        const askForCode = () => {
            this.rl.question(chalk.magenta('\n💻 Enter your code (or "mode" to switch):\n'), async (code) => {
                if (code.trim().toLowerCase() === 'mode') {
                    console.log(chalk.blue('🔄 Switching to search mode...'));
                    this.startUserInputSearch();
                    return;
                }

                if (code.trim().toLowerCase() === 'quit' || code.trim().toLowerCase() === 'exit') {
                ...
                if (!code.trim()) {
                    console.log(chalk.yellow('   Please enter some code to execute.'));
                    askForCode();
                    return;
                }
